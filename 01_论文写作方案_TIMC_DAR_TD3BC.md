...
若代码实现优先使用 GRU，也必须保持方法叙事不变，并在消融中比较 MLP、GRU、TCN。
...
Agent 必须通过消融证明门控有效，不能仅凭解释宣称其“保证安全”。
...
2. `TD3BC-TCN`：只加入时序编码器；
3. `TD3BC-TCN-Pred`：加入多步预测；
4. `Residual-TD3BC`：加入行为先验残差；
5. `Residual-TD3BC-Gate`：加入门控但无预测；
6. `DAR-TD3BC`：完整方法。

内部基线用于公平消融，官方报告值用于外部参照。不能只和文献数值比较而没有任何同代码基线。
...
- 在 NeoRL-2 Pipeline 上按标准回报和控制指标进行统一评估，并通过消融与扰动实验分析各模块。
